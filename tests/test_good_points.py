...
class TestGoodPoints(unittest.TestCase):
    ...
    def test_proj_length_horizontal(self):
        ...
        a = R3(0.0, 0.0, 0.0)
        b = R3(3.0, 4.0, 0.0)
        self.assertAlmostEqual(a.proj_dist(b), 5.0)  # √(9+16) = 5
    ...
    def test_all_good_polyedr(self):
        ...
        self.assertAlmostEqual(p.calc_good_edges_proj_sum(), expected, places=5)

    def test_mixed_polyedr(self):
        #Смешанный случай — часть вершин внутри, часть вне.
        p = Polyedr("data/test_mixed.geom")
        # 5 рёбер с хотя бы одной хорошей вершиной:
        # (1,3): √1.53, (1,4): √3.33, (2,3): √3.33, (2,4): √1.53, (3,4): 3.0
